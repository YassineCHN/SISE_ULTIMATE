...
def _render_summary_card(s: dict, t: dict, compact: bool = False) -> html.Div:
    ...
    if compact:
        return html.Div([
            html.Div([
                ...
                html.Span(f" · Score {s.get('score', 0)}",
    ...
    # Version complète (dernière session)
    return html.Div([
        # Header
        html.Div([
            html.Span(emoji, style={"fontSize": "36px", "marginRight": "12px"}),
            html.Div([
                html.Div(titre, style={"color": t["text"], "fontWeight": "700",
                                       "fontFamily": t["font"], "fontSize": "18px"}),
                html.Div([
                    html.Span(s.get("player_name", ""), style={"color": t["accent1"]}),
                    html.Span(" · ", style={"color": t["subtext"]}),
                    html.Span(s.get("game_id", "").upper(), style={"color": game_col}),
                    html.Span(" · ", style={"color": t["subtext"]}),
                    html.Span(f"Score : {s.get('score', 0)}", style={"color": t["accent2"]}),
                    html.Span(" · ", style={"color": t["subtext"]}),
        ...
        html.Div([
            html.Div([
                html.Div("🏆 " + cl_glob, style={"color": t["accent2"], "fontSize": "12px",
                                                   "marginBottom": "6px"}),
                html.Div("📊 " + cl_perso, style={"color": t["accent3"], "fontSize": "12px"}),
            ]),
        ], style={"background": t["bg"], "borderRadius": "6px", "padding": "12px",
                  "marginBottom": "16px"}),
        ...
        html.Div([
            html.Div([
                ...
                html.Ul([html.Li(p, style={"color": t["text"], "fontSize": "12px",
                                            "marginBottom": "4px"}) for p in pf],
                        style={"paddingLeft": "16px", "margin": "0"}),
            ], style={"flex": "1"}),
            html.Div([
                html.Div("📈 Axes d'amélioration", style={"color": t["accent3"], "fontSize": "11px",
                                                           "fontWeight": "700", "marginBottom": "8px"}),
                html.Ul([html.Li(a, style={"color": t["text"], "fontSize": "12px",
                                            "marginBottom": "4px"}) for a in axes],
                        style={"paddingLeft": "16px", "margin": "0"}),
            ], style={"flex": "1"}),
        ...
        html.Div([
            ...
            html.Span("Conseil : ", style={"color": t["accent1"], "fontWeight": "700",
                                            "fontSize": "12px"}),
...
def parse_url(search):
    """Lit les query params de l'URL (?player=X&game=Y) et navigue vers post-session."""
    if not search:
    ...
    try:
        ...
        if player and game:
            return {"player": player, "game": game}, "postsession", False
    except Exception:
        pass
    return {}, dash.no_update, True
...
def load_postsession_summary(n, params):
    ...
    try:
        from core.llm_summary import fetch_latest_summaries
        summaries = fetch_latest_summaries(limit=20)
        match = next((s for s in summaries
                      if s.get("player_name") == player and s.get("game_id") == game), None)
        if match:
            return match, True   # Stop polling — store the summary dict
    except Exception:
        pass
...
#     "Explorateur":     "Joystick stable, navigation prudente. Préfère explorer sans se presser.",
#     "Speedrunner":     "Joystick très actif, trajectoires directes, excellent score.",
#     "Sniper":          "Appuis longs et précis, bon score. Vise avant de tirer.",
#     "Rusheur":         "Appuis courts et fréquents, score variable. Fonce sans réfléchir.",
#     "Fantôme":         "Quasi inactif, score très faible. Peu d'engagement.",
#     "Réactif":         "Appuis fréquents et variés, bon score. Réflexes rapides.",
#     "Hésitant":        "Appuis rares et lents, score faible. Hésite avant chaque action.",
#     "Pilote maîtrisé": "Peu de corrections, trajectoires fluides, excellent score.",
#     "Pilote instable": "Corrections fréquentes, trajectoire chaotique, score faible.",
# }
# PROFILES_BY_GAME = {
#     "labyrinth": ["Explorateur", "Speedrunner"],
#     "shooter":   ["Sniper", "Rusheur", "Fantôme"],
#     "reflex":    ["Réactif", "Hésitant"],
#     "racing":    ["Pilote maîtrisé", "Pilote instable"],
# }
...
#     t            = THEMES[theme]
#     is_real      = df_real is not None
#     real_players = sorted(df_real["player_name"].unique().tolist()) if is_real else []
...
#                 html.Div("🤖 Agent Imitateur", style={"color": t["accent1"], "fontSize": "22px",
#                                                        "fontWeight": "700", "fontFamily": t["font"], "marginBottom": "4px"}),
#                 html.Div("L'IA joue à votre place en apprenant d'un profil ou d'un joueur réel",
#                          style={"color": t["subtext"], "fontSize": "13px"}),
#             ]),
#             data_badge(is_real, theme),
#         ], style={"display": "flex", "justifyContent": "space-between",
...
#                         {"label": "🏎️ Racing",    "value": "racing"},
#                     ],
#                     value="shooter", clearable=False,
#                     style={"background": t["bg"], "color": "#000", "borderRadius": "6px", "marginBottom": "16px"},
#                 ),
#                 html.Div("Mode d'imitation", style={"color": t["subtext"], "fontSize": "11px", "marginBottom": "8px"}),
#                 dcc.RadioItems(
#                     id="agent-mode-select",
#                     options=[
#                         {"label": "  🎭 Profil générique", "value": "profile"},
#                         {"label": "  👤 Joueur réel",      "value": "player"},
#                     ],
#                     value="player" if real_players else "profile",
#                     labelStyle={"display": "block", "marginBottom": "6px",
#                                 "color": t["text"], "fontSize": "13px", "cursor": "pointer"},
#                     style={"marginBottom": "16px"},
#                 ),
#                 html.Div(id="agent-profile-section", children=[
#                     html.Div("Profil à imiter", style={"color": t["subtext"], "fontSize": "11px", "marginBottom": "6px"}),
#                     dcc.Dropdown(
#                         id="agent-profile-select",
#                         options=[{"label": p, "value": p} for p in PROFILES_BY_GAME["shooter"]],
#                         value="Sniper", clearable=False,
#                         style={"background": t["bg"], "color": "#000", "borderRadius": "6px", "marginBottom": "12px"},
#                     ),
#                 ]),
#                 html.Div(id="agent-player-section", children=[
#                     html.Div("Joueur à imiter", style={"color": t["subtext"], "fontSize": "11px", "marginBottom": "6px"}),
#                     dcc.Dropdown(
#                         id="agent-player-select",
#                         options=[{"label": f"👤 {p}", "value": p} for p in real_players],
#                         value=real_players[0] if real_players else None,
#                         placeholder="Lance d'abord une partie..." if not real_players else "Choisir un joueur...",
#                         clearable=False,
#                         style={"background": t["bg"], "color": "#000", "borderRadius": "6px", "marginBottom": "8px"},
#                     ),
#                     html.Div(
#                         "⚠️ Aucun joueur réel — lance d'abord une partie" if not real_players
#                         else f"✅ {len(real_players)} joueur(s) disponible(s)",
#                         style={"color": t["accent3"] if not real_players else t["accent2"],
#                                "fontSize": "10px", "marginBottom": "12px"},
#                     ),
#                 ]),
#                 html.Div(id="agent-profile-desc", style={
#                     "color": t["subtext"], "fontSize": "11px", "fontStyle": "italic",
#                     "marginBottom": "16px", "padding": "8px",
#                     "borderLeft": f"2px solid {t['accent2']}",
#                     "minHeight": "32px",
#                 }),
#                 html.Button("▶ LANCER L'AGENT", id="btn-launch-agent", n_clicks=0, style={
#                     "background": t["gradient"], "border": "none", "color": "#000",
...
#                     "letterSpacing": "2px", "width": "100%",
#                 }),
#                 html.Div(id="agent-launch-feedback", style={"marginTop": "12px", "fontSize": "12px"}),
#                 html.Div(style={"height": "4px"}),
#                 html.Div("💡 Appuie sur Echap dans le jeu pour stopper l'agent", style={
...
#                     html.Div("Comparaison Humain vs Agent IA", style={"color": t["subtext"], "fontSize": "11px",
#                                                                         "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "12px"}),
#                     html.Div(id="agent-comparison-graph",
#                              children=dcc.Graph(figure=make_agent_comparison(theme),
#                                                 config={"displayModeBar": False})),
#                 ], theme, {"marginBottom": "16px"}),
#                 make_card([
#                     html.Div("Score de similarité comportementale", style={"color": t["subtext"], "fontSize": "11px",
#                                                                              "textTransform": "uppercase", "letterSpacing": "2px", "marginBottom": "16px"}),
#                     html.Div(id="agent-similarity-display", children=[
#                         html.Div("—", style={"color": t["subtext"], "fontSize": "48px",
#                                               "fontFamily": t["font"], "fontWeight": "700", "textAlign": "center"}),
#                         html.Div("Lance l'agent pour calculer la similarité",
#                                  style={"color": t["subtext"], "fontSize": "12px",
...
# @app.callback(
#     Output("agent-profile-select",  "options"),
#     Output("agent-profile-select",  "value"),
#     Output("agent-player-select",   "options"),
#     Output("agent-player-select",   "value"),
#     Output("agent-profile-section", "style"),
#     Output("agent-player-section",  "style"),
#     Input("agent-game-select",  "value"),
#     Input("agent-mode-select",  "value"),
#     dash.dependencies.State("sessions-store", "data"),
# )
# def update_agent_selectors(game_id, mode, sessions_data):
#     profiles  = PROFILES_BY_GAME.get(game_id, [])
#     p_options = [{"label": p, "value": p} for p in profiles]
#     p_value   = profiles[0] if profiles else None
#     real_players = []
#     if sessions_data:
#         try:
#             df = pd.DataFrame(sessions_data)
#             df = df[df["game_id"] == game_id]
#             real_players = sorted(df["player_name"].unique().tolist())
#         except Exception:
#             pass
#     pl_options = [{"label": f"👤 {p}", "value": p} for p in real_players]
#     pl_value   = real_players[0] if real_players else None
#     show, hide = {"display": "block"}, {"display": "none"}
#     if mode == "profile":
#         return p_options, p_value, pl_options, pl_value, show, hide
#     else:
#         return p_options, p_value, pl_options, pl_value, hide, show


# @app.callback(
#     Output("agent-profile-desc", "children"),
#     Input("agent-mode-select",   "value"),
#     Input("agent-profile-select","value"),
#     Input("agent-player-select", "value"),
#     Input("agent-game-select",   "value"),
#     dash.dependencies.State("sessions-store", "data"),
# )
# def update_agent_desc(mode, profile_name, player_name, game_id, sessions_data):
#     if mode == "profile":
#         return PROFILE_DESCRIPTIONS.get(profile_name or "", "")
#     if not player_name:
#         return "Sélectionne un joueur pour voir ses stats."
#     try:
#         df = pd.DataFrame(sessions_data or [])
#         df = df[(df["game_id"] == game_id) & (df["player_name"] == player_name)]
#         if len(df) == 0:
#             return f"Aucune session pour {player_name} sur {game_id}."
#         n     = len(df)
#         score = df["score"].mean() if "score" in df.columns else 0
#         lx    = df["lx_std"].mean() if "lx_std" in df.columns else 0
#         btn   = df["btn_press_rate"].mean() if "btn_press_rate" in df.columns else 0
#         return f"{n} session(s) · Score moy. {score:.0f} · Joystick {lx:.2f} · Boutons {btn:.2f}"
#     except Exception:
#         return f"Stats de {player_name} indisponibles."


# @app.callback(
#     Output("agent-launch-feedback",    "children"),
#     Output("agent-launch-feedback",    "style"),
#     Output("agent-comparison-graph",   "children"),
#     Output("agent-similarity-display", "children"),
#     Input("btn-launch-agent", "n_clicks"),
#     dash.dependencies.State("agent-game-select",    "value"),
#     dash.dependencies.State("agent-mode-select",    "value"),
#     dash.dependencies.State("agent-profile-select", "value"),
#     dash.dependencies.State("agent-player-select",  "value"),
#     dash.dependencies.State("sessions-store",        "data"),
#     dash.dependencies.State("theme-store",           "data"),
#     prevent_initial_call=True,
# )
# def launch_agent(n_clicks, game_id, mode, profile_name, player_name, sessions_data, theme):
#     t      = THEMES[theme]
#     target = player_name if mode == "player" else profile_name
#     if not game_id or not target:
#         return ("⚠️ Sélectionne un jeu et une cible.",
#                 {"marginTop": "12px", "fontSize": "12px", "color": "#FFB800"},
#                 dash.no_update, dash.no_update)

#     data_path = os.path.join(ROOT_DIR, "data", "synthetic_sessions_500.csv")
#     if not os.path.exists(data_path):
#         data_path = os.path.join(ROOT_DIR, "synthetic_sessions_500.csv")

#     try:
#         cmd = [sys.executable, "main.py", game_id, "Agent_IA",
#                "--agent", target, "--mode", mode]
#         if os.path.exists(data_path):
#             cmd += ["--data", data_path]
#         subprocess.Popen(cmd, cwd=ROOT_DIR)
#         src_label      = "données réelles" if mode == "player" else "profil synthétique"
#         feedback_msg   = f"✅ Agent '{target}' lancé sur {game_id} ({src_label})"
#         feedback_style = {"marginTop": "12px", "fontSize": "12px", "color": "#00F5FF"}
#     except Exception as e:
...
#     agent_stats = {}
#     try:
#         if mode == "player" and sessions_data:
#             df = pd.DataFrame(sessions_data)
#             df = df[(df["game_id"] == game_id) & (df["player_name"] == target)]
#         elif mode == "profile" and os.path.exists(data_path):
#             df = pd.read_csv(data_path)
#             gcol = "profile" if "profile" in df.columns else "player_name"
#             df   = df[(df["game_id"] == game_id) & (df[gcol] == target)]
#         else:
#             df = pd.DataFrame()
#         if len(df) > 0:
#             agent_stats = {k: float(df[k].mean()) for k in
#                            ["btn_press_rate","lx_std","ly_std","input_regularity","score"]
#                            if k in df.columns}
#     except Exception:
#         pass
...
#             df = df[df["game_id"] == game_id]
#             if len(df) > 0:
#                 human_stats = {k: float(df[k].mean()) for k in
#                                ["btn_press_rate","lx_std","ly_std","input_regularity","score"]
#                                if k in df.columns}
#         except Exception:
#             pass
...
#     # Bar chart
#     cats = ["Fréq. boutons", "Agitation LX", "Agitation LY", "Régularité"]
#     keys = ["btn_press_rate", "lx_std", "ly_std", "input_regularity"]
#     maxv = {"btn_press_rate": 1.0, "lx_std": 1.0, "ly_std": 1.0, "input_regularity": 10.0}
#     h_vals = [min(1.0, (human_stats.get(k) or 0) / maxv[k]) for k in keys]
#     a_vals = [min(1.0, (agent_stats.get(k) or 0) / maxv[k]) for k in keys]
#     fig = go.Figure()
#     fig.add_trace(go.Bar(name="Humains",           x=cats, y=h_vals, marker_color=t["accent2"], opacity=0.85))
#     fig.add_trace(go.Bar(name=f"Agent ({target})", x=cats, y=a_vals, marker_color=t["accent1"], opacity=0.85))
#     fig.update_layout(
#         barmode="group",
#         paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
#         font=dict(color=t["text"], family=t.get("font_body","monospace")),
#         xaxis=dict(showgrid=False),
#         yaxis=dict(showgrid=True, gridcolor=t["border"], range=[0,1]),
#         legend=dict(bgcolor="rgba(0,0,0,0)"),
#         margin=dict(l=20,r=20,t=10,b=20), height=250,
#     )
#     comparison = dcc.Graph(figure=fig, config={"displayModeBar": False})

#     # Similarité
#     if human_stats and agent_stats:
#         diffs = [abs((human_stats.get(k) or 0) - (agent_stats.get(k) or 0)) /
#                  max(abs(human_stats.get(k) or 0), abs(agent_stats.get(k) or 0), 0.001)
#                  for k in keys]
#         sim = round(max(0, min(100, (1 - sum(diffs)/len(diffs)) * 100)), 1)
#         col = t["accent2"] if sim >= 70 else (t["accent3"] if sim >= 40 else "#FF4C6A")
#         similarity = [
#             html.Div(f"{sim}%", style={"color": col, "fontSize": "48px",
#                                         "fontFamily": t["font"], "fontWeight": "700", "textAlign": "center"}),
#             html.Div(f"de similarité · Agent '{target}' vs humains sur {game_id}",
#                      style={"color": t["subtext"], "fontSize": "13px", "textAlign": "center", "marginTop": "4px"}),
#             html.Div(html.Div(style={"width": f"{sim}%", "height": "8px",
...
#             html.Div("—", style={"color": t["subtext"], "fontSize": "48px",
#                                   "fontFamily": t["font"], "fontWeight": "700", "textAlign": "center"}),
#             html.Div("Joue d'abord une partie pour comparer",
#                      style={"color": t["subtext"], "fontSize": "12px",
#                             "textAlign": "center", "marginTop": "4px"}),
#         ]

#     return feedback_msg, feedback_style, comparison, similarity
